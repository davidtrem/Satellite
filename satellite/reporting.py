...
class PreferenceDialog(QtGui.QDialog):
	...
	def __init__(self, parent=None):
		...
		okButton.connect(okButton,QtCore.SIGNAL("clicked()"),self,QtCore.SLOT("set_accept()"))
		cancelButton=QtGui.QPushButton("Cancel",self)
		cancelButton.connect(cancelButton,QtCore.SIGNAL("clicked()"),self,QtCore.SLOT("reject()"))
...
class ReportFrame(QtGui.QMainWindow):
	def __init__(self, my_adr, parent=None):
		...
		try:self.setWindowIcon(QtGui.Icon("icon.jpg"))
		except:pass
		...
		self.menusave = QtGui.QAction("&Save", self, shortcut=QtGui.QKeySequence.Save, statusTip="Save the Report", triggered=self.save_in_file)
		fileMenu.addAction(self.menusave)

		self.menuprint = QtGui.QAction("&Print", self, shortcut=QtGui.QKeySequence.Print, statusTip="Print the Report", triggered=self.print_file)
		...
		self.menupref = QtGui.QAction("Preference", self, statusTip="define the preference for the report", triggered=self.set_preference)
		editMenu = menubar.addMenu("&Edit") 
		editMenu.addAction(self.menupref)
		
		self.menuseuil = QtGui.QAction("Show Threshold Data", self, statusTip="show the threshold data used for data extraction", triggered=self.show_threshold)
		editMenu.addAction(self.menuseuil)
		
		self.menucss = QtGui.QAction("Set Report Style", self, statusTip="set css file to define the report style", triggered=self.set_css_file)
	...
	def save_in_file(self) :
		saveName = QtGui.QFileDialog.getSaveFileName(self,"Save Report", ".", "Report Files (*.doc)")
		if not saveName[0] == "":
	...
	def set_preference(self):
		...
		if preference_dialog.exec_() == QtGui.QDialog.Accepted:
			...
			#print preference_dialog.new_spot,preference_dialog.new_fail,preference_dialog.new_seuil
